...
def test_модуль_пира_не_может_быть_сколь_угодно_длинным():
    ...
    Проверка чужого модуля растёт квадратично по его длине и идёт под
    снятым GIL, поэтому `SIGINT` не доходит до процесса, пока она не
    вернётся: замерено 26 секунд глухоты на 32768-битном входе. Модуль
    приезжает от пира по проводу.
    """
