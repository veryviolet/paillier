...
игрушечном ключе `p = 23`, `q = 47`, где элементов 1078 и обойти их
можно все; выборка из двенадцати ключей на 256-битных простых; и один
...
|---|---|---|
| 1024 бита (сейчас) | 256 | 1433 мкс |
| 512 | 128 | 717 мкс |
| 256 | 64 | 356 мкс |

Полное последовательное шифрование на той же машине — 1449–1501 мкс на
элемент по трём прогонам (`benches/bench_rust.py`), медиана 1457. Значит
постоянная часть, от длины показателя не зависящая, составляет
**24–68 мкс**, то есть меньше пяти процентов. Отсюда:

| показатель | полное время | эл/с в потоке |
|---|---|---|
| 1024 бита | 1457 мкс | 686 |
| 512 | 741–785 мкс | 1270–1350 |
| 256 | 380–424 мкс | **2360–2630** |

При `heu` 1378 эл/с по ядру сокращение до 256 бит действительно
**обгоняет его**, а не отстаёт вдвое.

Два предостережения к этим числам, и оба существенны.

Во-первых, они ВЫВЕДЕНЫ вычитанием, а не измерены сквозным прогоном:
длина показателя снаружи не настраивается, она жёстко следует из `|n|`.

Во-вторых, постоянная часть — это разность двух независимо замеренных
величин около 1450 мкс каждая, поэтому её собственная неопределённость
сравнима с ней самой: по трём прогонам вышло от 24 до 68 мкс. Диапазон
в таблице выведен из этой вилки честно: `356 + [24, 68]` даёт
`380–424` мкс, то есть `2360–2630` эл/с.

Здесь стояло `2500–2650` — нижний край на 6 % оптимистичнее, чем
следует из мною же названного разброса. Диапазон, посчитанный небрежно,
хуже одного числа: одно число хотя бы не притворяется оценкой
неопределённости.
