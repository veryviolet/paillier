...
fn main() {
    for key_bits in [2048u32, 3072] {
        ...
        println!(
            "  итого разбор + умножение   {:6.2} мкс/оп   (в add_many замерено 85)",
            (parse.as_secs_f64() + plain.as_secs_f64()) * 1e6 / ROUNDS as f64,
        );

        // Точная копия цикла `add_many`: свои байты у каждого
        // слагаемого, разбор и умножение — как в бою. Нужна потому, что
        // замеры выше дают 6.9 мкс на операцию, а `add_many` показывает
        // 85, и разницу надо объяснить, а не назвать.
